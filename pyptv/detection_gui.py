...
class DetectionGUI(HasTraits):
    ...
    def _button_manual_fired(self):
        ...
        if points_set:
            ...
            self.status_text = "Set 4 points on each Detection image for manual orientation"
    ...
    def _button_sort_grid_fired(self):
        ...
        Detection points in the fixp target file and the targets
    ...
    def _button_raw_orient_fired(self):
        """
        update the external Detection with results of raw orientation, i.e.
        ...
        for i_cam in range(self.n_cams):
            ...
            success = external_Detection(self.cals[i_cam], selected_points, \
    ...
    def _button_fine_orient_fired(self):
        ...
        for i_cam in range(self.n_cams): # iterate over all cameras
            ...
            if self.epar.Combine_Flag:
                ...
                self.status_text = "Multiplane Detection."
                """ Performs multiplane Detection, in which for all cameras the 
                ...
                for i in range(self.MultiParams.n_planes): # combine all single planes
                    ...
                    # Load Detection point information from plane i
                    try:
                ...
                # Make into the format needed for full_Detection.
            ...
            try: 
                residuals, targ_ix, err_est = full_Detection(self.cals[i_cam], self.cal_points['pos'], \
                                                           targs, self.cpar, flags)
            except:
                raise ValueError("full Detection failed\n")
            # save the results
            self._write_ori(i_cam)
    ...
    def _write_ori(self, i_cam):
        """ Writes ORI and ADDPAR files for a single Detection result
        """
    ...
    def save_point_sets(self, i_cam):
        """
        Saves detected and known Detection points in crd and fix format, respectively.
        These files are needed for multiplane Detection.
        """
