...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        exp_path = next((path for path in candidate_paths if path.exists()), None)
        if exp_path is None:
            raise FileNotFoundError(
                "No default experiment directory found. Pass a YAML file or a valid experiment directory."
            )
        exp = Experiment()
        exp.populate_runs(exp_path)
        yaml_file = exp.active_params.yaml_path
